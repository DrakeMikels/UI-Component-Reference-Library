...
const DesignSystemCardPreview = ({ system }: { system: DesignSystemEntry }) => {
  const r = radiusPx[system.radius];
  const btnR = `${r}px`;
  const inputR = `${Math.min(r, 6)}px`;

  return (
    <div className="mb-4 h-28 overflow-hidden border border-ink-200 bg-ink-50 dark:border-ink-700 dark:bg-ink-950 sm:h-32">
      {/* Nav bar */}
      <div
      ...
      <div className="px-3 pt-2">
        <div
          ...
          className="flex h-6 items-center gap-1.5 border border-ink-300 bg-white px-2 dark:border-ink-600 dark:bg-ink-900"
        >
...
export const DesignSystemCard = ({ system, onSelect }: DesignSystemCardProps) => {
  ...
  return (
    ...
    >
      ...
      <DesignSystemCardPreview system={system} />
